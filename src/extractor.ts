...
// Supponiamo che tu abbia una funzione che recupera i link grezzi
async function fetchRawStreams(type: string, id: string): Promise<Stream[]> {
    // ... qui dentro c'è la tua logica di scraping per ottenere i link ...
    // Per esempio, restituisce un array di oggetti stream base:
    return [
        { name: "VixSrc FHD", url: "https://example.com/stream1.m3u8" },
        { name: "VixSrc HD", url: "https://example.com/stream2.m3u8" }
    ];
}

export async function getStreamContent(type: string, id: string, showBothLinks: boolean): Promise<Stream[]> {
    const rawStreams = await fetchRawStreams(type, id);
    if (!rawStreams || rawStreams.length === 0) {
        return [];
    }

    const finalStreams: Stream[] = [];
    const proxyUrl = process.env.MFP_PROXY_URL;

    if (!proxyUrl) {
        console.error("URL del proxy non trovato! Controlla il file .env");
        return rawStreams; // Restituisce i link originali come fallback
    }

    for (const stream of rawStreams) {
        // Se showBothLinks è true, aggiungi prima il link originale
        if (showBothLinks) {
            finalStreams.push(stream);
        }

        // Crea e aggiungi sempre il link con proxy
        const proxyStream: Stream = {
            ...stream, // Copia tutte le proprietà (es. behaviorHints)
            name: `${stream.name}\n(Proxy)`, // Aggiunge "(Proxy)" al nome
            url: `${proxyUrl}${encodeURIComponent(stream.url)}` // Crea l'URL del proxy
        };
        finalStreams.push(proxyStream);
    }

    // Se showBothLinks è false, l'array conterrà solo i link proxy.
    // Se è true, conterrà sia gli originali che i proxy.
    return finalStreams;
}

// La tua funzione per estrarre l'URL originale, ora nel posto giusto.
function extractOriginalUrl(proxyUrl: string): string {
  ...
  } catch (e) { /* Ignora l'errore se non è un URL valido */ }
  return proxyUrl;
}
...
// La funzione principale che ora contiene TUTTA la tua logica
export async function getStreamContent(id: string, type: 'movie' | 'series', showBothLinks: boolean): Promise<Stream[]> {
    const streamResults = await fetchStreamsFromSource(id, type);
    ...
    for (const st of streamResults) {
        ...
        if (showBothLinks) {
            finalStreams.push({ name: "StreamViX", title: contentTitle, url: originalUrl, behaviorHints: { notWebReady: true } });
            if (mfpProxyUrl) {
                finalStreams.push({ name: "StreamViX (Proxy)", title: contentTitle, url: mfpProxyUrl, behaviorHints: { notWebReady: false } });
            }
        } else {
            if (mfpProxyUrl) {
                finalStreams.push({ name: "StreamViX (Proxy)", title: contentTitle, url: mfpProxyUrl, behaviorHints: { notWebReady: false } });
